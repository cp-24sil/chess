...
export interface Piece {
  ...
  isWhite: boolean;
  move(posX: number, posY: number): boolean;
  showRoutes(x: number, y: number, isWhiteTurn: boolean): void;
}

interface ElemTableau {
  colorIndex: string;
  piece: Piece | "";
}
...
const knightMoves: MoveDirection[] = [
  { dx: -2, dy: -1 }, { dx: -2, dy: 1 },
  { dx: 2, dy: -1 }, { dx: 2, dy: 1 },
  { dx: -1, dy: -2 }, { dx: -1, dy: 2 },
  { dx: 1, dy: -2 }, { dx: 1, dy: 2 }
];
const kingMoves: MoveDirection[] = [
...
function imgName(name: string): string {
  const map: Record<string, string> = { t: "rook", c: "knight", f: "bishop", Q: "queen", K: "king", p: "pawn" };
...
function showRoutes(x: number, y: number): void {
...
function isKingAttacked(x: number, y: number, isWhiteTurn: boolean): boolean {
  ...
  for (const m of knightMoves) {
    ...
    if (nx >= 0 && nx < 8 && ny >= 0 && ny < 8) {
      const p = plateau[ny]![nx]!.piece;
      if (p !== "" && p.isWhite !== isWhiteTurn && p.name === "c") return true;
    }
  }
  ...
  if (pawnRow >= 0 && pawnRow < 8) {
    ...
    if (x - 1 >= 0 && leftPiece && leftPiece.name !== "" && leftPiece.isWhite !== isWhiteTurn && leftPiece.name === "p") return true;

    const rightPiece = plateau[pawnRow]![x + 1]?.piece;
    if (x + 1 < 8 && rightPiece && rightPiece.name !== "" && rightPiece.isWhite !== isWhiteTurn && rightPiece.name === "p") return true;
  }
  for (const m of kingMoves) {
    ...
    if (nx >= 0 && nx < 8 && ny >= 0 && ny < 8) {
      const p = plateau[ny]![nx]!.piece;
      if (p !== "" && p.isWhite !== isWhiteTurn && p.name === "K") return true;
    }
  }
  return false;
}
...
function hasAnyLegalMove(isWhiteTurn: boolean): boolean {
  for (let y = 0; y < 8; y++) {
    for (let x = 0; x < 8; x++) {
      const p = plateau[y]![x]!.piece;
