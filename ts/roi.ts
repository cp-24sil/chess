...
export class roi extends piece {
    ...
    if (app.plateau[posY]![posX]!.colorIndex == "lightblue" || app.plateau[posY]![posX]!.colorIndex == "lightcoral") {
      ...
      if (Math.abs(posX - fromX) === 2) {
        ...
        if (rookPiece !== "") {
          ...
          (rookPiece as roi).hasMoved = true;
        }
      }
  ...
  showRoutes(x: number, y: number, isWhiteTurn: boolean) {
    ...
    for (let m of this.moves) {
      ...
      if (app.plateau[ny]![nx]!.piece !== "" && app.plateau[ny]![nx]!.piece.isWhite === isWhiteTurn) continue;
  ...
  tryRoque(x: number, y: number, isWhiteTurn: boolean, direction: number) {
    if (app.isKingAttacked(x, y, isWhiteTurn)) return;

    let rookX = direction === 1 ? 7 : 0;
    let rookPiece = app.plateau[y]![rookX]!.piece;
    if (rookPiece === "" || rookPiece.name !== "t" || (rookPiece as roi).hasMoved) return;
    let startX = Math.min(x, rookX) + 1;
    let endX = Math.max(x, rookX) - 1;
    for (let i = startX; i <= endX; i++) {
      if (app.plateau[y]![i]!.piece !== "") return;
    }
